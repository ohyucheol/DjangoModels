...
class UserCreateForm(forms.ModelForm):
    ...
    # 즉, ModelForm의 경우 form의 validation, model의 validation에서 각각 검증되는 것으로 추측된다.
    ...
    cls2 = forms.EmailInput(attrs={'class': 'form-control mb-3'})
    cls3 = forms.PasswordInput(attrs={'class': 'form-control mb-3'})
    
    # 새로 정의한 field의 경우 기본적으로 required 속성을 가지며 입력 여부가 validation의 대상이 된다.
    # email의 경우 required 속성에 의하여 한 번, EmailValidator()에 의하여 한 번,
    # 총 두 번의 required error가 발생한다.
    username = forms.CharField(widget=cls1, error_messages=err1, validators=[val1])
    email = forms.EmailField(widget=cls1, validators=[val2], error_messages=err2)
    password1 = forms.CharField(widget=cls3, error_messages=err3)
    password2 = forms.CharField(widget=cls3, error_messages=err3)
